...
def fetch_all_rows(supabase, table_name):
    ...
    while True:
        response = supabase.table(table_name)\
            .select("HHID, SCREENTIME, K2Q31A, K2Q31C, SC_AGE_YEARS, SC_SEX, FWC, STRATUM, FIPSST, AGE_GROUP")\
...
model_cols = ['ADHD_CLINICAL', 'SCREENTIME', 'AGE', 'SEX', 'FWC']
...
h1a_clinical = smf.glm(
    'ADHD_CLINICAL ~ SCREENTIME + AGE + C(SEX)',
    data=df_clean,
    family=Binomial(),
...
model_cols_h1b = ['ADHD_CLINICAL', 'SCREENTIME', 'AGE_GROUP', 'SEX', 'FWC']
...
h1b_clinical = smf.glm(
    '''ADHD_CLINICAL ~ SCREENTIME * C(AGE_GROUP, Treatment(reference="0-5")) + C(SEX)''',
    data=df_clean_h1b,
    family=Binomial(),
...
# H1b — predicted probabilities by age group
fig, ax = plt.subplots(figsize=(8, 5))
...
# H1b — relative % increase from <1 hr baseline
fig, ax = plt.subplots(figsize=(8, 5))
